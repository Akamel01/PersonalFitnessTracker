...
import { ActivityType, Activity, WorkoutPlan, WeightLiftingLog } from '../types';
...
// Memoize Set row to prevent re-renders on other set inputs changing
const SetRow = memo(({ exIndex, setIndex, set, handleSetChange, removeSet }: { exIndex: number, setIndex: number, set: {reps: string, weight: string}, handleSetChange: Function, removeSet: Function }) => (
...
const WeightLiftingLogger: React.FC<Omit<LogFormProps, 'type'>> = ({ addActivity, workoutPlans, activities, onLogComplete }) => {
    ...
    // Add a single exercise field for freestyle mode
    const [freestyleExercise, setFreestyleExercise] = useState('');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        workoutData.forEach(log => {
             // For freestyle, use the dedicated state for the exercise name
            const exerciseName = selectedPlanId === 'freestyle' ? log.exerciseName : log.exerciseName;

            if (exerciseName && log.sets.some(s => s.reps && s.weight)) {
                addActivity({
                    type: ActivityType.WEIGHT_LIFTING,
                    exercise: exerciseName,
                    sets: log.sets.filter(s => s.reps && s.weight).map(s => ({ reps: parseInt(s.reps), weight: parseFloat(s.weight) })),
            ...
            {workoutData.map((exerciseLog, exIndex) => (
                <div key={exIndex} className="p-4 border border-gray-700 rounded-lg space-y-3">
                    {selectedPlanId === 'freestyle' ? (
                        <>
                            ...
                            <datalist id="exercise-list">
                                {exerciseDictionary.map(ex => <option key={ex.name} value={ex.name} />)}
                            </datalist>
                        </>
                    ) : (
                    ...
                    {exerciseLog.sets.map((set, setIndex) => (
                        <SetRow key={setIndex} exIndex={exIndex} setIndex={setIndex} set={set} handleSetChange={handleSetChange} removeSet={removeSet} />
                    ))}
